...
focusdataR1 <- read.csv("InformativenessVsDifficultyR1 - FOCUS.csv", row.names=1)
controldataR1 <- read.csv("InformativenessVsDifficultyR1 - Control.csv", row.names=1)
p_startsR1 = c(1,5,9,13)

focusdataR2 <- read.csv("InformativenessVsDifficultyR2 - FOCUS.csv", row.names=1)
controldataR2 <- read.csv("InformativenessVsDifficultyR2 - Control.csv", row.names=1)
p_startsR2 = c(1,5,9,15)



create_graphs = function(round_no,focusdata,controldata,p_starts,do_sqrt){
  ...
  if (round_no==2){
    ...
    if(do_sqrt){
      ...
      plot(focus_difficulty,focus_informativeness_Qx,main="Round 1 Informativeness by Question vs. Difficulty (All WRPS square rooted)",
           col = 2,xlab = 'Difficulty: Average(WRPS)',ylab = 'Informativeness: Cor(WRPSx,Avg(WRPS~x))',
           xlim=c(0,1.25),ylim=c(-.5,1),pch=16)
    } else {
      png("Round 1 Informativeness by Question vs. Difficulty",width = 1000,height = 700)
      plot(focus_difficulty,focus_informativeness_Qx,main="Round 1 Informativeness by Question vs. Difficulty",
           col = 2,xlab = 'Difficulty: Average(WRPS)',ylab = 'Informativeness: Cor(WRPSx,Avg(WRPS~x))',
           xlim=c(0,1.25),ylim=c(-.5,1),pch=16)
    }
    ...
    if(do_sqrt){
      ...
      plot(focus_difficulty,focus_informativeness_Px,main="Round 1 Informativeness by Problem vs. Difficulty (All WRPS square rooted)",
           col = 2,xlab = 'Difficulty: Average(WRPS)',ylab = 'Informativeness: Cor(WRPSy,Avg(WRPS~y))',
           xlim=c(0,1.25),ylim=c(-.5,1),pch=16)
    } else {
      png("Round 1 Informativeness by Problem vs. Difficulty",width = 1000,height = 700)
      plot(focus_difficulty,focus_informativeness_Px,main="Round 1 Informativeness by Problem vs. Difficulty",
           col = 2,xlab = 'Difficulty: Average(WRPS)',ylab = 'Informativeness: Cor(WRPSy,Avg(WRPS~y))',
           xlim=c(0,1.25),ylim=c(-.5,1),pch=16)
    }
...
create_graphs(1,focusdataR1,controldataR1,p_startsR1,T)
create_graphs(1,focusdataR1,controldataR1,p_startsR1,F)
create_graphs(2,focusdataR2,controldataR2,p_startsR2,T)
create_graphs(2,focusdataR2,controldataR2,p_startsR2,F)
